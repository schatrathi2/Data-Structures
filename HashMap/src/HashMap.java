...
public class HashMap<K, V> {
    ...
    public V remove(K key) {
        ...
        MapEntry prev = table[i];
        if (prev.getNext() != null) {
            MapEntry curr = prev.getNext();
            while (curr != null) {
                if (curr.getKey().equals(key)) {
                    found = true;
        ...
        if (!found) {
            throw new NoSuchElementException("The key is not in the hash map.");
        }
        return null;
    }
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("length is less than the size.");
        }

        MapEntry[] newTable = new MapEntry[length];
        for (MapEntry<K, V> e : table) {
            while (e != null) {
                MapEntry<K, V> next = e.getNext();
                int i = e.getKey().hashCode() % length;
